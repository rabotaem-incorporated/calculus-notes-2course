...
#th(name: "Фреше", label: "frechet")[
    Пусть $f: RR^n --> RR$ и $f$ измерима#rf("def-mfn") относительно меры Лебега. Тогда сущесвует последовательность $f_n$ непрерывных функций, сходящаяся к $f$ почти везде#rf("def-ae-converge").
]
