...
#proof[
    $mu E{abs(f_n - f) >= 1/k} --> 0$. Берем $n_k > n_(k - 1)$ так, что $ mu underbrace(E{abs(f_n_k - f) >= 1/k}, := A) <= 1/2^k $
    ...
    Проверим, что вне $B$ будет поточечная сходимость. $B_1 supset B_2 supset B_3 supset ... $. Если $x in.not B$, то $x in.not B_n$ для некоторого $n$. Значит $x in.not A_k forall k > n$, и $abs(f_(n_k) - f(n)) < 1/k forall k > n ==> lim f_(n_k) (x) = f(x).$
]
...
#th(name: "Фреше")[
    Пусть $f: RR^n --> RR$ и $f$ измерима относительно меры Лебега. Тогда суещсвует последовательность $f_n$ непрерывных функций, сходящаяся к $f$ почти везде.
]
...
#th(name: "Лузина")[
    Пусть $f: E subset RR^m --> R$ измерима относительно меры Лебега. Тогда для любого $eps > 0$ существует $A subset E$, такое что $mu(E without A) < eps$ и $f bar_A$ непрерывна.
]
