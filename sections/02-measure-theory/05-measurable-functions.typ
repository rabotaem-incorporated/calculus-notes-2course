...
#proof[
    Хотим понять, что $E{F < a} =^rf("def-mfn") F^(-1)(-oo, a) = f^(-1)(phi^(-1)(-oo, a))$ измеримо. $phi^(-1)(-oo, a) =: U$ --- открытое множетсво в $H$, так как $phi$ непрерывна. Тогда $U = H sect G$, где $G$ --- открыто в $RR^m$. Тогда $f^(-1)(U) = f^(-1)(H sect G) = f^(-1)(G)$. Остается доказать, что если $G subset RR^m$ открыто, то $f^(-1)(G)$ измеримо.

    Открытые множества представляются в виде счетного объедининия ячеек#rf("borel-set-over-cells"):
...
#follow[ 
    В условии теоремы в качестве $phi$ можно взять поточечный предел непревыных функций#rf("inf-sup-lim-mfn")#rf("mfns", "continious").
]
...
#th(label: "simple-approx")[
    Пусть $f: X --> overline(RR)$, $f >= 0$ измеримая#rf("def-mfn"). Тогда существует последовательность простых функций#rf("def-simple-fn") $phi_n: X --> RR$, $0 <= phi_1 <= phi_2 <= ...$. и $f = lim phi_n$ поточечно. Если $f$ ограниченая, то $phi_n$ можно выбрать так, что $phi_n arrows.rr f$#rf("def-fn-converge") на $X$.
]
