...
#proof[
  //#TODO[Нужно вставить сюда ахуительную картинку из пиздатого конспекта]
  $
    x in G ==> x in overline(B)_r(x) subset G.
  $
  #figure[
    #image("../../images/pupailupa.svg", width: 50%)
  ]
  Найдем $R_x$ --- параллелепипед, что $Cl R_x subset B_r(x) subset G$.
  Тогда $union.big_(x in G) R_x = G$.
  #TODO[Не полное доказательство, надо дописать/переписать]

  Зато есть явный алгоритм: построить "решетку", взять все ячейки которые полностью внутри $G$, потом поделить решетку пополам, взять снова, ну и так далее. Хз, мне очевидно, надо просто расписать нормально.
]
...
#proof[
  ...
  3. Знаем, что ячейка --- счетное пересечение открытых множества, значит, что все ячейки содержаться в $Bb^m ==> Bb(Pp^m) subset Bb^m$
]
