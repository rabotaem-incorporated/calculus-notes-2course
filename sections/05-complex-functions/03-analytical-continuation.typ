...
#example[
  ...
  Так как $abs(n^z) = n^(Re n)$, при $Re z > 1$, ряд абсолютно сходится. Попробуем продифференцировать,
  $
  ...
  $
  А штука справа голоморфна, при $Re z > 0$, так как $1/(2z - 1)^z - 1/(2z)^z sim z/(2n)^(z + 1)$. Для производной тоже будет сходимость (опустим эти вычисления).
