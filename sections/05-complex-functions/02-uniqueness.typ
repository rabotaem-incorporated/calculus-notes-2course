...
#lemma(label: "opened-closed-trivial")[
...
#th(name: "о единственности", label: "uniqueness")[
...
#th(name: "о среднем", label: "value-through-average")[
...
#proof[
  Запишем интегральную формулу Коши#rf("cauchy-integral"):
  $
...
#follow(label: "value-through-average'")[
...
#th(name: "Принцип максимума", label: "max-principle")[
...
#proof[
  Пусть $M = abs(f(a))$ --- максимум по модулю. Если $M = 0$, то функция равна нулю в окрестности, и по теореме о единсвенности#rf("uniqueness"), везде. 

  Если $M > 0$, то домножим функцию на константу вида $e^(-i alpha)$ так, чтобы $f(a) = M$. Значение в точке станет положительным. Запишем интегральную формулу Коши#rf("cauchy-integral"):
  $
  ...
  $
  Снова везде равентсва. Аналогично получается, что $Re f(a + r e^(i t)) = M$. Получается, и модуль, и вещественная часть везде равна $M$. Тогда для каждой точки круга, $f(a + r e^(i t)) = M space forall r <= R space forall t$, а значит $f(z) = M$ для всех $z$ в круге, и по теореме о единственности#rf("uniqueness"), $f = const$.
]

#follow(label: "max-hfn-is-on-bound")[
...
#def[
  Пусть $f in H(Omega)$, $a in Omega$. $a$ --- _нуль голоморфной функции_ $f$, если $f(a) = 0$.
]

#th(label: "hfn-zero-multiplicity")[
...
#proof[
  Рассмотрим разложение $f(z) = sum_(n = 0)^oo a_n (z - z_0)^n$, в нем не все $a_n$ нули. Возьмем в качетстве $m := min {k: a_k != 0} >= 1$ --- минимальный индекс, в котором значение не $0$ (напомню, что $a_0$ равно нулю, так как $z_0$ --- корень, и $m >= 1$). 
  Тогда 
  $
  ...
  $
  Надо проверить голоморфность. При $z != z_0$ очевидно: это степенной ряд. При $z = z_0$, это предел функции $f(z)/(z - z_0)^m$ при $z -> z_0$, значит функция непрерывна. У нас была теорема, что если непрерывная функция голоморфна без точки, то она все равно голоморфна везде#rf("hfn-without-line-hfn"). Значит $g$ голоморфна везде. 
  
  Наконец, $g(z) != 0$ в окрестности $z_0$ по непрерывности, значит $f(z) != 0$ в проколотой окрестности $z_0$.
]

#follow(label: "hfn-prod-zero-hfn-zero")[
  Пусть $f, g in H(Omega)$, и $f dot g equiv 0$. Тогда либо $f equiv 0$, либо $g equiv 0$.
]

#proof[
  Пусть $g equiv.not 0$. Берем $z_0 in Omega$ такое, что $g(z_0) != 0$. Тогда $f(z_0) = 0$, и $f(z) != 0$ в $circle(U)_(z_0)$ по предыдущей теореме#rf("hfn-zero-multiplicity"). Тогда $g equiv 0$ в $circle(U)_(z_0)$, так как $f dot g equiv 0$. Значит это нуль везде по теореме о единсвенности#rf("uniqueness").
]

#follow(label: "hfn-zeros-isolated")[
  Множество нулей голоморфной функции $f equiv.not 0$ состоит из изолированных точек.
]

#proof[
  Очевидно по теореме#rf("hfn-zero-multiplicity"): около каждого нуля есть проколотая окрестность, в которой функция не равна нулю.
]

#th(name: "о единственности, вторая версия", label: "uniqueness2")[
...
#proof[
  ...
  Но тогда $z_0$ не изолированный нуль $h$, значит $h equiv 0$ в $Omega$#rf("hfn-zeros-isolated").
]
