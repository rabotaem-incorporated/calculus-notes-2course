...
#ticket[Производящие функции. Операции с производящими функциями. Задача о размене.]
...
#def[
    ...
    Естественно, мы хотим чтобы у этой функции был хоть какой-то радиус сходимости, то есть чтобы коэффициенты не росли быстрее чем $O(n^c)$.
]
...
#ticket[Производящая функция для числа разбиений натуральных чисел. Теорема Эйлера.]
...
#ticket[Диагонализация степенных рядов.]
...
#ticket[Произведение Адамара рациональных функций. Способ вычисления произведения Адамара.]
...
#proof[
    ...
    - "$<==$": Достаточно понять для одного "коэффициента", $a_n = p(n) q^n$. Представим многочлен в виде
        ...
        $
            (1/(1 - q z))' = sum_(n = 1)^oo n q^n z^(n - 1) = sum_(n = 0)^oo (n + 1) q^(n + 1) z^n.
        $
]
