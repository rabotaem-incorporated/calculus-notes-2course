...
#proof[
    ...
    $
        ...
        product_(n = 1)^oo 1/(1-z^n) product_(n = 1)^oo (1-z^(2 n)) =
        product_(n = 1)^oo (1-z^(2n))/(1-z^n) =
        product_(n = 1)^oo (1 + z^n),
    $
    а это число разбиений на различные слагаемые.
]
...
#example(name: "диагонализация степенных рядов")[
    ...
    Такая производящая функция по двум переменным иногда пишется много проще, чем по одной. Например, для биномиальных коэффициентов:
    $
    ...
    $
    В таком ряде коэффициент перед $z^0$ это $sum_(m = 0)^oo a_(m m) w^m$. А мы умеем находить такой коэффициент: поделим ряд на $z$ и найдем вычет, действительно:
    $
    ...
    $
        z^2 - z + w = 0 ==> w = (1 plus.minus sqrt(1 - 4w))/2.
    $
    Нас интересуют все корни, для которых при маленьких $w$ (при $w < r/2$, а $r$ можно выбирать сколь угодно маленьким) мы попадаем в круг сходимости, поэтому при знаке "$+$" мы точно в круг сходимости не попадем. Значит
    $
        sum res =
        res (1 - sqrt(1 - 4w))/2 =
...
#proof[
    - "$==>$": $Aa(z)$ --- рациональная, значит ее можно представить как линейная комбинация простейших (вида $1/(z - a)^k$). Надо научиться раскладывать ее
        $
    ...
    - "$<==$": Достаточно понять для одного "коэффициента", $a_n = p(n) q^n$. Представим многочлен в виде
        ...
        $
            a_n = (n + k) (n + k - 1) ... (n + 1) q^(n + k),
        $
        для $k = 0$, $a_n = q^n$ и 
        $
            1/(1 - q n) = sum_(n = 0) q^n z^n,
        $
...
#th[
    Произведение Адамара рациональных функций --- радиональная функция.
]
...
#example(name: "метод Дарбу")[
    Наша цель: оценивать скорость роста коэффициентов происводящих рядов.
    
    Пусть имеется фукнция $f(z) = sum_(n = 0)^oo a_n z^n$, $R$ --- конечный радиус сходимости (если ряд скходимости бесконечность, вряд ли такой ряд нужен в комбинаторике, где обычно последовательности целочислены, если его коэффициенты убывают так быстро). Тогда на границе круга сходимости есть особая точка.
    #TODO[ниче не понятно, но очень интересно]
...
#example[
    ...
    $
        f(z) - e/sqrt(1 - z) = underbrace((e^z - e)/sqrt(1 - z), sum_(n = 0)^oo b_n z^n) = e dot (e^(z - 1) - 1)/(sqrt(1 - z)) = e dot (sum_(k = 1)^oo c_k (z - 1)^k)/(sqrt(1 - z)).
    $
    Тогда
    $
