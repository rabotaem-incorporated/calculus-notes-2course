...
#proof[
    ...
    $
        abs(integral_(C_eps) c/z dif z) <= M dot (beta - alpha) eps --> 0,
    $
...
#example[
    ...
    $
    В таком интеграле можно вытащить мнимую часть из под знака интеграла (это нечетная функция), и тогда
    $
        I =
    ...
    $
    В третьем интеграле вычет равен $1$, так как можно разложить $e^(i x)$ в ряд как $1 + x + ...$, и коэффициент при минус первом члене ряда будет равен $1$. Знак минус там из-за того, что контур обходится по часовой стрелке.
...
#example[
    ...
    $
        integral_(Gamma_R) e^((p - 1) Ln z)/(1 + z) dif z = 2 pi i res_(z = -1) e^((p - 1) Ln z)/(1 + z) = 2 pi i e^((p - 1) Ln (-1)) = e^(i pi (p - 1)).
    $
...
#th[
    ...
    В частности, мероморфная функция полностью опеределяется своими особами точками.
]
...
#proof[
    ...
    $
        ...
        2pi R max_(abs(xi) = R) abs((G_k (xi))/(xi - z)) = 2 pi R O(1/R^2) --> 0.
    $
    Значит
    $
...
#proof[
    ...
    #TODO[ну, блин, ну не... у всего (неубывающего) есть предел...]
