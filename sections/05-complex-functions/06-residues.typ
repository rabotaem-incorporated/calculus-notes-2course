...
#proof[
    ...
    $
    Сумма абсоютно сходится в кольце $r <= abs(z) <= r$, так как оно внутри кольца сходимости ряда, и мы просто домножили эту штуку на фигню, не влияющую на сходимость #text(red,)[(?)] #TODO[вычитать]. Тогда можем менять местами сумму и интеграл.
    $
        i integral_0^(2 pi) sum_(n = -oo)^oo c_n r^(n + 1) e^(i (n + 1) t) dif t =
    ...
    $
        i integral_0^(2 pi) e^(i (n + 1) t) dif t =
        cases(
...
#proof[
    ...
    $
    Нужен коэффициент при $z^(m - 1)$ в этом ряде. Это просто формула Тейлора. Доопределим $f(z)$ в нуле, и выпишем коэффициент:
    $
        c_(-1) = (g^(m - 1)(0))/(m - 1)! = 1/(m - 1)! lim_(z -> 0) z^m f(z).
    $
]
...
#proof[
    ...
    Тогда
    $
        z dot (A - f(z)) = -sum_(n = 1)^oo c_n/(z^n) dot z = -sum_(n = 1)^oo c_n/z^(n - 1).
    $
]
...
#proof[
    ...
    Тогда
    $
        res_(z = 0) 1/z^2 f(1/z) = res_(z = oo) = c_(-1).
    $
]
