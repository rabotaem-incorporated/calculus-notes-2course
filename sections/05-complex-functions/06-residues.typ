...
#example[
    ...
    $
        sum_(n = 1)^oo 1/n^2.
    $
    В общем случае, мы хотим смотреть на сумму как на сумму вычетов в каком-то комплексном интеграле. Сумма ищется по всем натуральным числам. У какой функции полюсы тоже находятся в натуральных числах? $ctg (pi z)$, например. И если мы считаем сумму $f(n)$, то так удачно получается, что
    $
        res_(z = n) ctg(pi z) dot f(z) = lr((cos (pi z) dot f(z))/(sin (pi z))' |)_(z = n) = f(n)/pi.
    $
    И если $f$ голоморфна в окрестности $z = n$, то
    $
        integral_(abs(z) = n+1/2) ctg(pi z) f(z) dif z = 2 pi i (sum_(k = -pi)^n (f(k))/pi + res_"остальные\nточки\nкруга" f(z)).
    $
    ...
    $
    Нужен коэффициент при $z$ у $ctg(pi z)$. Пишем ряды:
    $
        ctg(pi z) = cos(pi z)/sin(pi z) = (1-(pi z)^2/2 + O(z^4))/(pi z (1 - (pi z)^2/6 + O(z^4))) = 1/(pi z) (1-(pi z)^2/2 + O(z^4)) dot (1 + (pi z)^2/6 + O(z^4)) newline(=)
    ...
    Значит,
    $
        sum_(k=1)^oo 1/k^2 = - pi/2 dot res_(z = 0) ctg(pi z)/z^2 = -pi/2 -pi/3 = pi^2/6.
    $
]
...
#example[
    ...
    $
    Надо проверить, что $1/(sin pi z)$ ограничен на окружности $abs(z) = n + 1/2$. Давайте поверим в это на слово.
    ...
    Для очень большого числа рядов этот метод подойдет. Но, вот для обратных кубов, скажем, нет, потому что это нечетная функция и вычеты друг друга сократят. А вот все четные степени посчитаются, хотя возиться придется больше.
]
...
#proof[
    1. Очевидно.
    2. $Ln f$ --- первообразная вдоль пути $C$ у $f'/f$, с параметризацией $gamma: [a, b] --> CC$. То есть
        $
...
#example[
    Докажем, что уравнение $z + e^(-z) = lambda > 1$ в правой полуплоскости имеет ровно одно решение.
...
#exercise[
    Вывесли основную теорему алгебры из теоремы Руше. Указание: надо рассмотреть $f(z) = z^n$, а $g(z)$ --- все остальное.
]
