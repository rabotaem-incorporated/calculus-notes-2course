...
#example[
    ...
    $ 
    при $e^(2pi i alpha) = 1$, то есть $alpha = pi / n$. Тогда особая точка в этой области всего одна: $z = e^(i pi/(2 n))$.
    $
        integral_(Gamma_R) f(z) dif z = 2 pi i res f.
    $
    Интеграл есть
    ...
    $
    Первый интеграл сходится к $I$, второй интеграл сходится к 0, как мы показали выше, а третий интеграл сходится к $I dot -e^((pi i)/n)$, так как мы просто подставляем $z = t e^(i pi/n)$ в интеграл и разворачиваем отрезок. Через вычеты, интеграл равен
    $
