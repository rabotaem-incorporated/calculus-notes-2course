...
#follow[
    ...
    3. Если $a$ --- изолированная особая точка $f$ (неустранимая) и $f$ однолистна в проколотой окрестности $a$, то $a$ --- полюс первого порядка.
...
#proof[
    ...
    3. Пусть $a$ --- существенная особая точка. Тогда по теореме Сохоцкого, замыкание множества значений в окрестности --- все $overline(CC)$. Возьмем $z_0$ близко к $a$, и $f(z_0) = b$. Тогда можно рассмотреть небольшую окрестность $a$ радиуса $2r$, в которой не лежит $z_0$. А еще $f(0 < abs(z) < r)$ --- открытое множество (образ области --- область). В замыкании этого образа лежит $b$. С другой стороны, $f(abs(z - z_0) < r)$ --- тоже открытое. Из однолистности следует, что
        $ f(abs(z - z_0) < r) sect f(0 < abs(z) < r) = nothing ==> f(0 < abs(z) < r) subset CC without f(abs(f - f_0) < r). $
        Справа написано замкнутое множество, значит и ее замыкание лежит в нем:
        $
            CC = Cl f(0 < abs(z) < r) subset CC without f(abs(f - f_0) < r) subset CC without {b}.
        $
        Противоречие.
        Значит $a$ --- полюс. Рассмотрим $g(z) = 1/f(z)$ --- голоморфную функцию в окрестности $a$. Тогда $g(z)$ --- однолистна в окрестности $a$, следовательно $f'(a) != 0$. Значит $а$ --- нуль первого порядка $g$, и $a$ --- полюс первого порядка $f$. 
