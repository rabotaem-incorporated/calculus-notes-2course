...
#notice[
    ...
    Получается наш комплексный логарифм определен не однозначно, так как если все значения сдвинуть на $2pi k$, получится другой логарифм, для которого тоже $e^(g(z)) = z$. С другой стороны, разумно положить, что это полная арифметическая функция.
]
...
#notice[
    Все эти $Ln$ из одного класса эквивалентности.
]
#proof[
    #TODO[картиночка аналитических продолжений]
...
#props[
    ...
    2. Для полной аналитической функции,
        ...
        но это свойство нарушается, если говорить об отдельных представителях. Например, если рассмотреть такой $Ln$, что $Ln (-1) = pi i$, а $Ln 1 = 0$, то $ Ln ((-1)^2) = Ln 1 = 0 != Ln (-1) + Ln (-1) = 2pi. $
...
#notice[
    ...
    1. $p in ZZ$:
        ...
        Значит для $p in NN$ новых значений нет (разве что мы немного подпортили область определения, кажется (?)), короче соответсвие есть, все ок. Для отрицательных тоже все будет нормально.
    ...
    3. $p in CC without QQ$. Можно рассмотреть счетное число $k$ таких, что $e^(2pi i p k) = e^(2pi i p tilde(k))$, так как равенство есть если $2 pi i p (k - tilde(k)) = 2 pi i l ==> p = l / (k - tilde(k))$.
