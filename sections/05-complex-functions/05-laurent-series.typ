...
#follow[
    Если $f$ голоморфна в кольце $r < abs(z) < R$, то сущесвует $g in H(abs(z) < R)$ м $h in H(abs(z) > R)$, такие что $f = g + h$. Если $lim_(z -> oo) h(z) = 0$, то разложение единственно.
]
...
#proof[
    ...
    $
    Тогда $g$ сходится в кольце $r < abs(z) < R$, а значит и в $abs(z) < R$ (нет проблем со сходимостью около нуля), а значит $g in H(abs(z) < R)$. Тоже самое для $h$: она сходится при $abs(z) > r$, а значит голоморна при $abs(z) > R$.
...
#def[
    ...
    - Если не существует $lim_(z -> a) f(z)$, то такая точка называется _существенно особой точкой_.
]
