...
#props[
    1. Сущесвуют $0 <= r <= R  <= +oo$ такие, что ряд Лорана $sum_(n = -oo)^oo c_n z^n$ сходится при $r < abs(z) < R$ и расходится при $abs(z) < r$ или $abs(z) > R$.
]
...
#th(name: "Сохоцкого")[
    Пусть $a$ --- существенно особая точка $f$ и $eps > 0$. Тогда $ Cl {f(z): 0 < abs(z - a) < eps} = CC. $
...
#proof[
    ...
    2. Случай $A in CC$.
        ...
        - $a$ --- устранимая. Тогда сущесвует $lim_(z -> a) g(z) = b in CC$ и $lim_(z -> a) f(z) = lim_(z -> a) 1/(g(z)) + A$. Это либо бесконечность (если $b = 0$), либо комплексное число (если $b != 0$). Но такого быть не может: тогда $a$ --- либо устранимая особая точка, либо полюс $f$, но она должна быть существенной.
        - $a$ --- полюс $g$. Тогда $lim_(z -> a) g(z) = oo$ и $lim_(z -> a) f(z) = 1/oo + A = 0 + A = A$. Значит это устранимая особая точка, а не сущесвунная. Снова противоречие.
        - $a$ --- существенная особая точка $g$. Тогда сущесвует последовательность $g(z_n) -> oo$ по предыдущему пункту. А значит $f(z_n) = 1/(g(z_n)) + A --> A$.
]

#th(name: "Пикара")[
    Если $a$ --- существенно особая точка и $eps > 0$, то множество ${f(z): 0 < abs(z - a) < eps}$ это либо $CC$, либо $CC$ без одной точки.
]

#proof[
    Храбров сказал, что на доказательство уйдет пару или две, и мы доказываль ее не будем. Еще он сказал, что эту теорему иногда называют "большой теоремой Пикара", так как у нее есть ослабленный аналог, на доказательство которого уйдет всего лишь одна пара. Оно вам надо?
]
...
#def[
    Пусть $f: Omega --> C$, где $Omega subset overline(CC)$. Тогда если $lim_(z -> oo) f(z) = f(oo)$, то $f$ непрерывна в $oo$. 
]

#def[
    Пусть $f in H(abs(z) > R)$ ($f$ голоморфна в проколотой окрестности бесконечности).
    1. Если сущесвует $lim_(z -> oo) f(z) in CC$, то $oo$ --- устранимая особая точка.
...
#notice[
    ...
    1. $oo$ --- устранимая особая точка $f$ тогда и только тогда, когда $0$ --- устранимая особая точка $f(1/z)$, что равносительно тому, что $f(z)$ ограничена в окрестности $oo$, что равносильно тому, что в разложении $f$ в ряд Лорана по степеням $z$ при положительных степенях все коэффициенты равны нулю.

    2. $oo$ --- полюс $f$ тогда и только тогда, когда $0$ --- полюс $f(1/z)$, что равносительно тому, что в разложении $f$ в ряд Лорана по степеням $z$ при положительных степенях есть конечное ненулевое число ненулевых коэффициентов.

    3. $oo$ --- существенная особая точка $f$ тогда и только тогда, когда $0$ --- существенная особая точка $f(1/z)$, что равносительно тому, что в разложении $f$ в ряд Лорана по степеням $z$ при положительных степенях бесконечно много ненулевых коэффициентов.
]
...
#proof[
    $f$ --- голоморфна в $oo$, значит сущесвует $lim_(z -> oo) f(z) =: b$. Тогда сущесвует $R$ такое, что $abs(f(z) - b) < 1$ при $abs(z) > R$, в частности $abs(f(z)) < abs(b) + 1$ при $abs(z) > R$. В круге $abs(z) <= R$ функция ограничена как непрерывная на компакте. Значит, она ограничена везде, и по теореме Лиувилля, $f = const$.
]
...
#def[
    ...
    Северный полюс расположен в $N = (0, 0, 1)$, и рассматривается точка на плоскости $(x, y, 0)$. Южный полюс сферы лежит на плосткости в точке $(0, 0, 0)$.
    #TODO[картинка жесткого луча через северный полюс]
...
#proof[
    Параметризуем прямую $N z$: $(t x, t z, 1 - t)$. Уравнение сферы $u^2 + v^2 + (w - 1/2)^2 = 1/4$, то есть $u^2 + v^2 + w^2 = w$. Подставляем параметризацию прямой:
    $
...
#proof[
    - "$==>$", $z_n --> z_* in CC$: Тогда $abs(z_n - z_*) --> 0$, а значит
        $
            abs(z_n - z_*)/(sqrt(1 + abs(z_n)^2) sqrt(1 + abs(z_*)^2)) < abs(z_n - z) --> 0.
        $
    - "$<==$":
        $
            (z_n - z_*)/(sqrt(1 + abs(z_n)^2) sqrt(1 + abs(z_*)^2)) --> 0 ==> abs(z_n - z_*)/sqrt(1 + abs(z_n)^2) --> 0.
        $
        Если $z_n$ --- ограничена, то $abs(z_n - z_*) --> 0$, и все доказано. Если $z_n$ неограничено, то найдется $z_(n_k) --> oo$, и дробь
        $
