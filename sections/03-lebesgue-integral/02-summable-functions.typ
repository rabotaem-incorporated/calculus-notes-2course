...
#def[
    $f: E --> CC$ _измериная_ если $Re f$ и $Im f$ измеримы.
]

#def[
    Пусть $f: E --> CC$. Тогда коплексно-значным интегралом назовем $ IE f dif mu = IE Re f dif mu + i IE Im f dif mu $
...
#proof[
    ...
    $ INT(E: X, sum_(k = 1)^n bb(1)_(E_k) f)= INT(S_n, E: X) --> INT(bb(1)_E f, E: X) = INT(E: X, f). $
    Более того,
    $
...
#proof[
    Считаем $f, g >= 0$ (неотрицательность, модуль --- одно и тоже). Пусть $A^p := INT(f^p)$, $B^q := INT(g^q)$. Если $A = 0$, то $f = 0$, значит $f g = 0$, значит $INT(f g) = 0$. Аналогично, если $B = 0$.
    ...
    $ u v <= u^p / p + v^q / q, "где" u, v in R_(>= 0). $
