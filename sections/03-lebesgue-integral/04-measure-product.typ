...
#def[
    ...
    $
    Отметим, что если функция принимает значение $plus.minus oo$, то оно не принадлежит графику.
]

#lemma[
    Будем рассматривать меру $m := mu times lambda_1$.

    Если $f$ измерима, то $m Gamma_f (E) = 0$.
]
...
#lemma[
    Если $f$ измерима в широком смысле, то $Pp_f (E)$ измерим.
]
...
#proof[
    ...
    - "$<==$": Примерим принцип Кавальери для $Pp_f$.
        $
...
#proof[
    ...
    *Шаг 3*. 
    ...
    2. $phi_n (x) := integral_Y (psi_n)_x dif nu$ измерима в широком смысле и по теореме Леви сходится к $integral_Y f_x dif nu$.
