...
#proof[
    1. Пусть $S_n := sum_(k = 1)^n f_k(x)$, тогда $0 <= S_1 <= S_2 <= ...$. $S_n --> S := sum_(n = 1)^n f_n$. По теореме Леви 
...
#notice[
    Равенства может не быть. Если $f_n := n dot bb(1)_[0, 1/n]$, то
    $
...
#th(name: "Лебега о предельном переходе (о мажорируемой сходимости)")[
    Пусть $abs(f_n) <= F$, а $F$ суммируемо на $E$, и $f_n --> f$ почти везде. Тогда $lim INT(f_n) = INT(f)$. Более того, $INT(abs(f_n - f)) --> 0$.
    
    $F$ называется _суммируемая мажоранта_.
]

#proof[
    Пусть $h_n := 2F - abs(f_n - f) <= 2F$. Тогда $f_n --> 2F$ почти везде. $h_n >= 2F - abs(f_n) - abs(f) >= 0$. По усилинному варианту теоремы Леви,
    $
        INT(2F) - INT(abs(f_n - f)) = INT(h_n) --> INT(2F).
    $
    Отсюда $INT(abs(f_n - f)) --> 0$ (при сокращении использовали суммируемость). В частности, $ abs(INT(f_n) - INT(f)) = abs(INT(abs(f_n - f))) >= INT(abs(f_n - f)) --> 0. $
]

#exercise[
    Сверху есть замечание про $f_n = bb(1)_[0, 1/n]$. Показать, что у такой функции нет суммируемой мажоранты.
]
...
#proof[
    ...
    Аналогично,
    $
        g^*(x) = max_(t in [x_(k - 1), x_k]) f(t) "если" x in [x_(k - 1), x_k) ==> S_* = integral_[a, b] g^* dif lambda.
    $
...
#th(name: "критерий Лебега для интегрируемости по Риману")[
    $f: [a, b] --> R$ ограничена. Тогда $f$ интегрируема по Риману на $[a, b]$ тогда и только тогда, когда множество точек разрыва имеет нулевую меру Лебега.
]
