...
#proof[
    ...
    Действительно, если продифференцировать $Phi^(-1) (Phi (x)) = id$ получится $Phi' dot (Phi^(-1))^(-1) (Phi) = E$, значит $det Phi' != 0$ так как иначе произведение тоже имело бы оперелитель $0$. Отмечу, что производная композиции сущесвует, так как по определению диффеоморфизма#rf("def-diffeomorphism") $Phi^(-1)$ --- непрерывно диффеоморфизма. 
