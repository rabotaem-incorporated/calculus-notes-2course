...
#proof[
    ...
    Действительно, если продифференцировать $Phi^(-1) (Phi (x)) = id$ получится $Phi' dot (Phi^(-1))^(-1) (Phi) = E$, значит $det Phi' != 0$ так как иначе произведение тоже имело бы оперелитель $0$. Отмечу, что производная композиции существует, так как по определению диффеоморфизма#rf("def-diffeomorphism") $Phi^(-1)$ --- непрерывно диффеоморфизма. 
...
#example(name: "да, все еще от меня, и все еще непонятный")[
    Можно просто расширить рассужение из леммы с двух функций на $n$. Доказательтсво получиться аналогично, просто будет больше букв и индексов. Но давайте сделаем по другому. Я покажу на примере, не строго, как получить это по индукции.
