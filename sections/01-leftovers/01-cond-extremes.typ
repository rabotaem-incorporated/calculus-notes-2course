...
#proof[
  ...
  Определим $h(y) = f(y, g(y))$. Уменьшим $W$ так, чтобы $y in W ==> (y, g(y)) in U$. Значит $h(b) = f(a) >= f(y, g(y)) = h(y)$, то есть $b$ --- точка локального максимума для $h ==> h'(b) = 0$.
...
#proof[
  ...
  $Phi(x) = 0$ --- компакт, поэтому наибольшее и наименьшее значения сущесвуют.
