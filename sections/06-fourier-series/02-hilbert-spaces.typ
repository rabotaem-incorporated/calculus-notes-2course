...
#def[
  ...
  2. $e_1$, $e_2$, ... --- _полная система_, если $z perp e_n$ для любого $n$, то $z = 0$.
...
#th(name: "ортагонализация Грамма-Шмидта")[
...
#proof[
  Доказательство конструктивное, поэтому вместе с ним прилагается конкретный способ такую систему построить. Такой процесс называется "_ортагонализацей Грамма-Шмидта_".
  
  Понятно, что любую систему векторов можно нормировать, поэтому построим ортогональную, а потом исправим ее. Пусть $f_1 = x_1$. Дальше ищем $f_n = x_n - a_1 f_1 - a_2 f_2 - ...$. Тогда, так как это просто линенйная комбинация,
  $ 
...
#exercise[
  Если $tilde(e_1)$, $tilde(e_2)$, ... --- другая ортонормированная система с тем же свойством ($forall n space Lin {e_1, e_2, ..., e_n} = Lin {x_1, x_2, ..., x_n}$), то $tilde(e)_n = lambda_n e_n$, где $lambda_n in RR$ (или $CC$), и $abs(lambda_n) = 1$.
]
...
#proof[
  ...
  $
    Lin {P_1, P_2, ...} = Lin {t_0, t_1, t^2, ...}
  $
  --- а это все многочлены. Надо доказать, что замыкание всех многочленов это $L^2$. Мы докажем только для многочленов из пунктов 1-3, да и то творчески, сославшись на теорему из будущего. Надо понять, что любую функцию из $L^2$ можно приблизить многочленом. Берем $eps > 0$ и $f in L^2$. Тогда найдется $g in L^2$ непрерывная такая, что $norm(f - g)_2 < eps$. По теореме Вейерштрасса (докажем позже, хотя на практиках уже), любую непрерывную функцию на отрезке можно равномерно приблизить многочленом. Тогда существует $h$ --- многочлен такой, что $norm(f - h)_oo < eps$. Знаем
  $
    norm(g - h)_2 <= (mu [a, b])^(1/2) norm(g - h)_oo < C eps,
  $
  в $L^2 ((a, b), mu)$.
]
...
#proof[
  Лень писать: запишите нормы как скалярные произведения и раскройте скобки.
]

#th(name: "о наилучшем приближении")[
  $A in H$ --- непустое, замкнутое, выпуклое; $x in H$. Тогда сущесвует единственный элемент наилучшего приближения.
]

#proof[
  Пусть $d := rho(x, A)$. Пусть $y, z in A$. Тогда $(y + z)/2 in A$ из-за выпуклости. Подставим в тожество параллелограмма $x - y$ и $x - z$:
  $
...
#th(name: "об ортогональной проекции")[
  Пусть $H$ --- гильберово, $L subset H$ --- замкнутое подпростанство, $x in H$. Тогда $x$ единственным образом представляется в виде суммы $x = y + z$, где $y in L$, а $z perp L$. Такой $y$ называется _ортогональной проекцией_, и этот $y$ --- элемент наилучшего приближения к $x$ в $L$.
]

#proof[
  Пусть $y$ --- элемент наилучшего приближения к $x$ в $L$, он сущесвует по предыдущей теореме. $z := x - y$. Надо проверить, что $x perp L$, остальное получилось само.
  ...
  $
    ...
    abs(lambda)^2 norm(lambda)^2 >= cj(lambda) dotp(z, l) + lambda cj(dotp(z, l)).
  $
...
#def[
  Пусть $L$ --- замкнутое подпространсво $H$. _Ортогональное дополнение_ $L^perp = {x in H: x perp L}$.
]

#def[
  Пусть $L$ --- замкнутое подпространсво $H$. Оператор ортогонального проецирования $P_L x = "ортогональная проекция" x "на" L$.
]
...
#def[
  Пусть $(X, rho)$ --- метрическое пространво. Назовем его _сепарабельным_, если в нем есть счетное, всюду плотное множество.
]

#examples[
  1. $RR^d$: $QQ^d$ --- счетное, всюду плотное.
  2. $L^p (RR^d, lambda_d)$: плотны ступенчатые функции с рациональными значениями и ступеньками по ячейкам с рациональными координатами вершин. Доказательтво остается читателю.
]

#th[
  В сепарабельном гильбертовом пространстве сущесвует не более чем счетный ортонормированный базис.
]

#proof[
  Рассматриваем ${x_n}$ --- счетное, всюду плотное множество. Просеим это множество, и получим подмножество ${y_n}$ --- линейно независимое: тогда $Lin {x_n} = Lin {y_n}$. Ортогонализуем Граммом-Шмидтом. $Lin {x_n} = Lin {y_n} = Lin {e_n}$. Тогда $H = Cl {x_n} subset Cl Lin {x_n} = Cl Lin {e_n}$.
]
...
#proof[
  Пусть $H$ --- сепарабельное Гильбертово. Рассмотрим его ортонормированный базис $e_1$, $e_2$, .... Он счетный, так как пространсво бесконечномерное. Сопоставим $x arrow.long.bar c_n (x) = dotp(x, e_n)/norm(e_n)^2$. Это линейное отображение, поэтому линейность сохраняется. Расстояния тоже:
  $
    norm(x)^2 =
