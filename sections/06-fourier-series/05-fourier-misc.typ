...
#notice[
  ...
  $
    S_n (f, x) = integral_(-pi)^pi f(x - t) D_n (t) dif t = f * D_n.
  $
  А значит (по линейности свертки, которую мы не обсуждали, но которая есть)
  $
    sigma_n (f, x) = (S_0 (f, x) + S_1 (f, x) + ... + S_n (f, x))/(n + 1) = 1/n f * Phi_n.
  $
]
...
#example[
  $1/n Phi_n$ --- усиленная аппроксимативная единица.
]
...
#th(name: "Вейерштрасса (другая, но на самом деле та же самая)")[
