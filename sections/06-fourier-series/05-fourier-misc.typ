...
#ticket[Аппроксимативная единица. Теорема об аппроксимативной единице (сходимость по норме в $L^p$ без доказательства).]
...
#proof[
  ...
  3. Если взять $delta > 0$ по $eps > 0$ из определения непрерывности в $x$, то $I_1$ оценивается так же. $I_2$ оценим так:
    ...
    $
    Первый множитель стремится к нулю так как $K_h$ --- усиленная аппроксимативная единица.

  2. Без доказательства. Надо рассмотреть функцию $g(t) = integral_(-pi)^pi abs(f(x - t) - f(x)) dif x$. Доказать, что $g in C_(2pi)$, затем сослаться на первый пункт, применив неравенство Гельдера, и все получится. Но в интересах времени, Храбров доказательство не привел.
]

#ticket[Суммирование рядов Фурье по Чезаро. Теорема Фейера. Следствия. Теоремы Вейерштрасса о приближении непрерывных функций.]
...
#proof[
  Применим предыдущую теорему для $K_h = 1/pi Phi_n$.
]
...
#follow(plural: true)[
  ...
  3. (*Теорема единственности*) Если $f in L_(2pi)^1$, и все коэффициенты Фурье $f$ равны нулю, то $f = 0$ почти везде.
...
#proof[
  ...
  4. Будем доказывать, что тригонометрическая система --- базис. Достаточно понять, что система --- полная, то есть если функция $g$ ортогональна любому элементу тригонометрической системы, то $g = 0$ почти везде. А это равносильно тому, что $c_k (g) = 1/(2pi) integral_(-pi)^pi g(x) e^(-i k x) dif x = 1/(2pi) dotp(g, e^(i k x)) = 0$. 
]
