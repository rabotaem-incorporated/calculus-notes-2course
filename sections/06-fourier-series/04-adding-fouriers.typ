...
#def[
  Рассмотрим ряд $sum_(k = 0)^oo a_k$ и положим $A_n := sum_(k = 0)^n a_k$. Будем говорить, что $S$ --- сумма ряда по Чезаро, если $S = lim_(n -> oo) (A_0 + A_1 + ... + A_n)/(n + 1)$. Обзначается $(c) (sum a_k)$.
]
...
#example[
  ...
  $
  Он расходится, так как члены ряда не стремяться к нулю.
  ...
  1. $Phi_n$ --- непрерывная четная $2pi$-периодическая фунция.
  ...
  Значит, если $t != 2n m$, то $("c") (1/2 + sum_(k = 1)^oo cos k t) = 0$.
]
