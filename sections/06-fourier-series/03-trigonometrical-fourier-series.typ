...
#th(name: "принцип локализации")[
  Пусть $f, g in L^1 [-pi, pi]$ и $f$ совпадает с $g$ в $(x - delta, x + delta)$. Тогда ряды Фурье для фукнций $f$ и $g$ в точке $x$ ведут себя одинаково. Более того, $S_n (f, x) - S_n (g, x) -->_(n -> oo) 0$.
]
...
#proof[
  ...
  $
    integral_0^pi abs(f_(x_0)^* (t))/t dif t.
  $
...
#example[
  ...
  #TODO[график]
  Посчитаем коэффициенты Фурье. Синусные, очевидно, равны нулю (функция нечетная). Для косинусных:
  $
    b_n =
    1/pi integral_0^pi (pi - x)/2 sin (n x) dif x =
    -1/pi integral_0^pi x sin (n x) dif x newline(=)
    -1/(2pi) (x (- lr((cos (n x))/n |)_0^(2pi) + integral_0^(2pi) (cos (n x))/n dif x) = 1/n.
  $
...
#example(name: "эффект Гиббса")[
  ...
  $
  Переход к пределу под $*$ можно делать, потому что у нас есть суммируемая мажоранта (придумайте сами)... Этот интеграл равен примерно $1.178...$ Получается, что такой всплеск, величиной примерно в 18%, всегда возникает в частичных суммах.
]
