...
#ticket[Представление $a_n cos n x + b_n sin n x$ в виде свертки. Лемма Римана–Лебега.]
...
#ticket[Оценки коэффициентов Фурье для равномерно непрерывных, липшицевых и дифференцируемых функций.]
...
#follow[
  ...
  $
  В частности, если $f$ дважды непрерывно дифференцируема, то ряд Фурье абсолютно сходится.
]
...
#ticket[Ядро Дирихле. Свойства. Три формулы для частичных сумм ряда Фурье. Следствие.]
...
#proof[
  ...
  $
    A_k (f, x) = cases(
      1/(2 pi) integral_(-pi)^pi f(x - t) dif t\, "при" k = 0,
...
#proof[
  ...
  $
    integral_delta^pi (sin (n + 1/2) t)/(2 sin t/2) (f(x + t) + f(x - t)) dif t -->_(n-->oo) 0
  $
  по лемме Римана-Лебега.
]

#ticket[Принцип локализации. Признак Дини. Следствия признака Дини.]
...
#proof[
  ...
  2. $
      integral_0^delta abs(f^*_x_0 (t))/t dif t <= integral_0^delta (abs(f(x_0 + t) - f(x_0 + 0))/t + abs(f(x_0 - t) - f(x_0 - 0))/t) dif t.
    $
...
#ticket[Ряд Фурье для функции $(pi-x)/2$.]
...
#example[
  ...
  $
    ...
    -1/(2pi) integral_0^(2pi) x sin (n x) dif x newline(=)
  ...
  $
  Можем найти знакочередующийся ряд для $x$:
  $
...
#ticket[Ряд Фурье для функции $sign$. Эффект Гиббса.]
...
#example(name: "эффект Гиббса")[
  ...
  $
    4/pi sum_(n = 1)^oo (sin ((2n - 1)x))/(2n - 1) = sign x "при" -pi < x < pi "(в нуле тоже, т.к. регулярная точка)."
  $
  ...
  $
  Минимальный положительный корень производной --- $pi/(2n)$, то есть как раз этот всплеск. Теперь посмотрим на значения там:
  $
    S_n (pi/(2n)) =
  ...
  $
  Переход к пределу под $*$ можно делать, потому что у нас есть суммируемая мажоранта --- 1 (проверьте сами). Этот интеграл равен примерно $1.178...$ Получается, что такой всплеск, величиной примерно в 18% (то есть 9%, если считать что разрыв у нас величины 2 в примере, а не 1), всегда возникает в частичных суммах.
]
