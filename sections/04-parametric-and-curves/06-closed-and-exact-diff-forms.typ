...
#def[
  $gamma: [a, b] --> Omega$ путь, $omega$ --- локально точная форма в $Omega$. $f: [a, b] --> RR$ назовем _первообразной $Omega$ вдоль пути_, если для любой точки $t in [a, b]$, у $gamma(t)$ существует окрестронсть $U_(gamma(t))$ и первообразная $F$ в $U_(gamma(t))$ такая, что $f(t) = F (gamma (t))$ при $tau$ близких к $t$.
...
#th[
  Первообразная в доль пути существует, и единственна с точностью до константы.
]
...
#def[
  Пусть $Gamma$ --- область. $gamma_0, gamma_1: [a, b] --> Omega$ и $gamma_0 (a) = gamma_1 (a)$, $gamma_0 (b) = gamma_1 (b)$. $gamma_0$ и $gamma_1$ называются _гомотопные путями с неподвижными концами_, если существует $gamma: [a, b] times [0, 1] --> Omega$, непрерывная, такая, что 
  $
...
#proof[
  ...
  Мы докажем, что $f(b, 0) = f(b, 1)$ и $f(a, 0) = f(a, 1)$. Проверим, что $f(a, u)$ --- локально постоянная функция. Берем $(a, u)$. Найдется $U_(gamma(a, u))$ и $F$ --- первообразная $omega$ в $U_(gamma(a, u))$ такая, что $f(tau, nu) = F(gamma(tau, nu))$ при $(tau, nu)$ близких к $(a, u)$, и $f(a, u) = F(gamma(a, nu)) = F(gamma_0 (a))$.
]
...
#proof[
  ...
  Возьмем точку $(a, u)$ и окрестность $U_(gamma(a, u))$ и $F$ --- первообразная $omega$ в $U_(gamma(a, u))$, такие, что $f(tau, nu) = F(gamma(tau, nu))$ при $(tau, nu)$ близких к $(a, u)$. Найдется $U_(gamma(b, u))$ (можно считать, что это $U_(gamma(a, u))$, кажется?.. андрей, проверь)), и $tilde(F)$ --- первообразная $omega$ в $U_(gamma(a, u))$, такие, что $f(tau, nu) = tilde(F) (gamma(tau, nu))$ при $(tau, nu)$ близких к $(b, u)$. Имеем, $f(b, nu) - f(a, nu) = tilde(F) (gamma(b, nu)) - F (gamma(a, nu)) = tilde(F) (gamma(a, nu)) - F(gamma(a, nu))$ --- это константа, так как разность двух первообразных в одной и той же точке.
]
