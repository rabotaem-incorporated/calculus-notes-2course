...
#def[
  $gamma: [a, b] --> Omega$ путь, $omega$ --- локально точная форма в $Omega$. $f: [a, b] --> RR$ назовем _первообразной $Omega$ вдоль пути_, если для любой точки $t in [a, b]$, у $gamma(t)$ существует окрестность $U_(gamma(t))$ и первообразная $F$ в $U_(gamma(t))$ такая, что $f(t) = F (gamma (t))$ при $tau$ близких к $t$.
