...
#proof[
  ...
  - "$3 ==> 1$": зафиксируем $a in Omega$ и $forall x in Omega$ проведем из $a$ в $x$ ломаную $gamma$ со звеньями параллельными осям. Она есть по лемме.
    Рассмотрим $F(x) := integral_gamma omega$. Этот интеграл не зависит от выбора $gamma$, так как если к одной кривой добавить в конец вторую перевернутую, получится ломаная из $a$ в $a$. Она, может быть, самопересекается, но если порезать ее точками пересечения (их не более чем счетно), получаться интегралы по замкнутым несамопересекающемся ломаным, которые, по $3)$, равны 0. Ну значит интегралы тоже равны.
    ...
    Рассмотрим интеграл:
    $
      integral_[...] omega = integral_0^t sum_(k = 1)^n f_k (gamma(tau)) underbrace(gamma'_k (tau), [k = 1]) dif tau = integral_0^t f_1(x_1 + tau, x_2, ..., x_n) dif tau.
    $
    Значит,
    $
      (diff F)/(diff x_1) (x) = lim_(t -> 0) 1/t integral_0^t f_1(x_1 + tau, x_2, ..., x_n) dif tau = f_1 (x).
    $
]
