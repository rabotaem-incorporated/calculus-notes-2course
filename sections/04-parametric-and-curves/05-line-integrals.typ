...
#th(name: "формула Грина", label: "green")[
  $Omega subset RR^2$ --- ограниченая на плоскости область#rf("def-region"), граница которой представляет собой конечное объединение непересекающихся простых кусочно-гладких замкнутых кривых. $P, Q: Cl Omega --> RR$ --- непрерывные, такие, что $(diff P)/(diff y)$ и $(diff Q)/(diff x)$ непрерывны на $Cl Omega$. Тогда
