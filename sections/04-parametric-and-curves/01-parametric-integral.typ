...
#def[
  Пусть $(X, Aa, mu)$ --- пространство с мерой. $T$ --- подмножество метрического пространства. $E_t in Aa$ и для любого $t in T$ функция $f: X times T --> overline(RR)$ с фиксированным $t$ измерима. Пусть $F (t) := integral_(E_t) f(x, t) dif mu(x). $ 
]
...
#notice[
  На вопрос 4 мы уже научились неплохо отвечать.
  $ integral_T F dif nu = integral_T integral_X f(x, t) dif mu(x) dif nu(t). $
  По теореме Тонелли#rf("tonelli")/Фубини#rf("fubini"), можем переставлять интералы.
]

#th(name: "ответ на вопрос 1", label: "parametric-limit")[
  Пусть $t_0$ --- предельная точка. Суммируемая $f$ такова, что $f(x, t) -->_(t -> t_0) g(x)$ во всех $x in X$. Существует $Phi: X --> overline(RR)$ суммируемая, и окрестность $U_(t_0)$ такая, что $abs(f(x, t)) <= Phi(x)$ для любого $x$ и $t in U_(t_0)$#rf("def-lebesgue-condition"). Тогда $ lim_(t -> t_0) integral_X f(x, t) dif mu(x) = integral_X g dif mu. $
]

#proof[
  Проверяем существование предела по Гейне. Рассмотрим $t_n -> t_0$. Обозначим $f_n (x) := f(x, t_n)$. Тогда при больших $n$ верно $t_n in U_(t_0)$, и можно применить теорему Лебега о предельном переходе#rf("lebesgue-limit"): $abs(f_n (x)) <= Phi(x)$ для каждого $x$ начиная с некоторого $m$.  По теореме имеем, $lim integral_X f_n dif mu = integral_X g dif mu$.
]

Обобщим свойство, которым мы пользовались в теореме.

#def(label: "def-lebesgue-condition")[
...
#def(label: "def-parametric-uniform-converge")[
...
#follow(label: "parametric-limit'")[
  (из теоремы о предельном переходе#rf("parametric-limit"))
  Пусть $mu X < +oo$ и $f(x, t) converges_(t -> t_0) g(x)$#rf("def-parametric-uniform-converge") (где $f$ суммируема) равномерно на $X$. Тогда $g$ суммируема, и $ lim_(t -> t_0) integral_X f(x, t) dif mu(x) = integral_X g dif mu. $
]

#proof[
  Возьмем $eps = 1$, по нему $delta > 0$ из определения и пусть $rho(s, t_0) < delta$ ($s != t_0$). Тогда $abs(f(x, s) - g(x)) < 1$ для любого $x in X$, значит $abs(g(x)) < 1 + abs(f(x, s))$ --- суммируемая мажоранта.
]
...
#follow(label: "parametric-continious")[
  Пусть $mu X < +oo$, $f(x, t)$ непрерывна по $t$ в точке $t_0$ для любого $x in X$, и удовлетворяет локальному условию Лебега#rf("def-lebesgue-condition") в точке $t_0$. Тогда $F(t) := integral_X f(x, t) dif mu(x)$ непрерывна в точке $t_0$.
]

#proof[
  Очевидно по теореме#rf("parametric-limit").
]
...
#lemma(label: "compact-prod")[
  Пусть $K$ и $L$ компакты в $X$ и $Y$. Тогда $K times L$ --- компакт в $(X times Y, d)$.
]

#proof[
  Проверяем секвенциальную компактность#rf("def-seq-compact"). Берем $(x_n, y_n) in K times L$. $x_n in K$, значит найдется $x_(n_k)$ сходится к точке из $K$, и $y_(n_k) in L$, поэтому найдется $y_(n_(k_l))$, которая сходится к точке из $L$. Тогда $(x_(n_(k_l)), y_(n_(k_l)))$ сходится к точке из $K times L$. Доказали.
]

#th(label: "parametric-continious-compact")[
  Пусть $X$ и $T$ компакты, $mu X < +oo$ и $f in C(X times T)$. Тогда $F(t) := integral_X f(x, t) dif mu(x) in C(T)$.
]

#proof[
  $X times T$ компакт по лемме#rf("compact-prod"), непрерывная функция на компакте ограничена#rf("weierstrass"). Значит $abs(f(x, t)) <= M$ для любого $x in X$ и $t in T$. Нашли мажоранту. Подставляем в следствие, получаем что хотим.
]

#follow(label: "parametric-continious-compact'")[
...
#th(label: "parametric-derivative")[
  Пусть $f: X times T --> RR$ суммируема для любого $t in T$, пусть $T subset RR$ --- промежуток. Пусть $f'_t (x, t)$ удовлетворяет локальному условию Лебега#rf("def-lebesgue-condition") в точке $t_0$. Тогда $F$ дифференцируема в точке $t_0$, и $F' (t_0) = integral_X f'_t (x, t_0) dif mu(x)$.
]
...
#proof[
  ...
  Надо понять, когда $integral_X g(x, h) dif mu(x) -->_(h -> 0) integral_X f'_t (x, t_0) dif mu(x)$. Мы знаем, что $g(x, h) -->_(h -> 0) f'_t(x, t_0)$, а когда можно переходить к интегралам? Когда выполняется локальное условие Лебега для $g$ при $h$ близких к $0$. Проверяем его. По теореме Лагранжа#rf("lagrange"),
  $ 
...
#follow(label: "parametric-derivative-compact")[
...
#th(name: "формула Лейбница", label: "leibniz")[
  Пусть $f: anyinterval(a, b) times anyinterval(c, d) --> RR$, пусть $f, f'_t in C(anyinterval(a, b) times anyinterval(c, d))$. Пусть $phi, psi: anyinterval(c, d) --> anyinterval(a, b)$ непрерывно дифференцируемые. Тогда если
  $
